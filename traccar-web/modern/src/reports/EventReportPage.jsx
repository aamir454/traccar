...
const EventReportPage = () => {
  ...
  useEffectAsync(async () => {
    if (selectedItem) {
      const response = await fetch(`/gps/api/positions?id=${selectedItem.positionId}`);
      if (response.ok) {
  ...
  useEffectAsync(async () => {
    const response = await fetch('/gps/api/notifications/types');
    if (response.ok) {
  ...
  const handleSubmit = useCatch(async ({ deviceId, from, to, type }) => {
    ...
    if (type === 'export') {
      window.location.assign(`/gps/api/reports/events/xlsx?${query.toString()}`);
    } else if (type === 'mail') {
      const response = await fetch(`/gps/api/reports/events/mail?${query.toString()}`);
      if (!response.ok) {
        throw Error(await response.text());
      }
    } else {
      setLoading(true);
      try {
        const response = await fetch(`/gps/api/reports/events?${query.toString()}`, {
    ...
    switch (key) {
      ...
      case 'attributes':
        switch (item.type) {
          ...
          case 'media':
            return (<Link href={`/gps/api/media/${devices[item.deviceId]?.uniqueId}/${item.attributes.file}`} target="_blank">{item.attributes.file}</Link>);
          case 'commandResult':
