...
const SummaryReportPage = () => {
  ...
  const handleSubmit = useCatch(async ({ deviceIds, groupIds, from, to, type }) => {
    ...
    if (type === 'export') {
      window.location.assign(`/gps/api/reports/summary/xlsx?${query.toString()}`);
    } else if (type === 'mail') {
      const response = await fetch(`/gps/api/reports/summary/mail?${query.toString()}`);
      if (!response.ok) {
      ...
      setLoading(true);
      try {
        const response = await fetch(`/gps/api/reports/summary?${query.toString()}`, {
