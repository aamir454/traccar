...
const StopReportPage = () => {
  ...
  const handleSubmit = useCatch(async ({ deviceId, from, to, type }) => {
    ...
    if (type === 'export') {
      window.location.assign(`/gps/api/reports/stops/xlsx?${query.toString()}`);
    } else if (type === 'mail') {
      const response = await fetch(`/gps/api/reports/stops/mail?${query.toString()}`);
      if (!response.ok) {
      ...
      setLoading(true);
      try {
        const response = await fetch(`/gps/api/reports/stops?${query.toString()}`, {
