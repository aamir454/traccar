...
const TripReportPage = () => {
  ...
  useEffectAsync(async () => {
    if (selectedItem) {
      ...
      const response = await fetch(`/gps/api/reports/route?${query.toString()}`, {
        headers: {
  ...
  const handleSubmit = useCatch(async ({ deviceId, from, to, type }) => {
    ...
    if (type === 'export') {
      window.location.assign(`/gps/api/reports/trips/xlsx?${query.toString()}`);
    } else if (type === 'mail') {
      const response = await fetch(`/gps/api/reports/trips/mail?${query.toString()}`);
      if (!response.ok) {
        throw Error(await response.text());
      }
    } else {
      setLoading(true);
      try {
        const response = await fetch(`/gps/api/reports/trips?${query.toString()}`, {
