...
const LoginPage = () => {
  ...
  const generateLoginToken = async () => {
    if (nativeEnvironment) {
      ...
      try {
        ...
        const response = await fetch('/gps/api/session/token', {
          method: 'POST',
  ...
  const handlePasswordLogin = async (event) => {
    ...
    try {
      const response = await fetch('/gps/api/session', {
        method: 'POST',
  ...
  const handleTokenLogin = useCatch(async (token) => {
    const response = await fetch(`/gps/api/session?token=${encodeURIComponent(token)}`);
    if (response.ok) {
      const user = await response.json();
      dispatch(sessionActions.updateUser(user));
      navigate('/');
  ...
  const handleOpenIdLogin = () => {
    document.location = '/gps/api/session/openid/auth';
  };
